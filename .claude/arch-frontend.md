...
│   ├── WorkshopForm.tsx # Formulário modal para oficinas (sem uploads, 3 seções: Dados, Endereço, Especialidades/Detalhes); sem criação de login — apenas referência; prop `onSave`: `(workshop: Partial<Workshop>) => Promise<void>`; CNPJ check contra `workshop_accounts` → sugere "Convidar Oficina"; validação de CNPJ duplicado na aplicação via pré-query antes do INSERT (garante erro 23505 com mensagem amigável: "Este CNPJ já está cadastrado para este cliente.")
│   ├── InviteWorkshopModal.tsx # Modal para gerenciar convites de oficina parceira; usa `invokeFn()` helper com `fetch()` direto + `supabase.auth.getSession()` para JWT (evita anon key do Supabase JS v2); chama funções `workshop-invitation` e `workshop-partnership-manage`
...
│   ├── Workshops.tsx    # CRUD de oficinas (Fleet Assistant+ acessa, Fleet Analyst+ edita, Manager+ deleta ou Fleet Analyst com flag) + botão Eye → WorkshopDetailModal; botão Edit com `title="Editar"` para seletor Playwright; pré-validação de CNPJ duplicado em saveMutation antes do INSERT
