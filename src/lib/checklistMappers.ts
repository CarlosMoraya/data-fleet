...
export interface ChecklistRow {
  ...
  vehicles?: { license_plate: string; driver?: { name: string } | null; shippers?: { name: string } | null; operational_units?: { name: string } | null } | null;
...
export function checklistFromRow(row: ChecklistRow): Checklist {
  return {
    ...
    vehicleShipperName: row.vehicles?.shippers?.name ?? undefined,
    vehicleOperationalUnitName: row.vehicles?.operational_units?.name ?? undefined,
