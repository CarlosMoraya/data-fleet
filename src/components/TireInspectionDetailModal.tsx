  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/60 p-4">
      <div className="my-8 w-full max-w-2xl rounded-2xl bg-white shadow-xl">
        ...
        {/* Meta */}
        <div className="grid grid-cols-2 gap-4 border-b px-6 py-4 text-sm sm:grid-cols-4">
        ...
        {/* Comparison */}
        <div className="p-6">
          <h3 className="mb-3 text-sm font-semibold text-zinc-800">Comparação (3 últimas inspeções)</h3>
          {loadingComparison || isLoading ? (
            <div className="flex justify-center py-12">
              <Loader2 className="animate-spin text-zinc-400" size={28} />
            </div>
          ) : comparisonError ? (
            <p className="py-8 text-center text-sm text-red-600">Não foi possível carregar a comparação.</p>
          ) : comparison.length === 0 ? (
            <p className="py-8 text-center text-sm text-zinc-400">Nenhuma foto registrada para esta inspeção.</p>
          ) : (
...
function SummaryBadge({ label, value, color }: { label: string; value: string | number; color: string }) {
  ...
  return (
    <div className={`flex flex-col items-center rounded-lg px-3 py-2 ${colorMap[color]}`}>
...
function ComparisonPhotoCard({
  ...
  return (
    <div className="w-40 flex-shrink-0 overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm">
      <div className="relative h-32 w-full bg-zinc-100">
        {photo.photoUrl ? (
          ...
          >
            <img src={photo.photoUrl} alt={positionCode} className="h-full w-full object-cover" />
          </button>
        ) : (
          <div className="flex h-full items-center justify-center text-xs text-zinc-300">Sem foto</div>
        )}
        ...
        {photo.isCurrent && (
          <span className="absolute top-1 left-1 rounded-full bg-orange-500 px-2 py-0.5 text-[10px] font-semibold text-white">
            Atual
          </span>
        )}
      </div>
