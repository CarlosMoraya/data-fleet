 ...
 * com intervalo livre a partir de 0 dias.
 */
...
test.describe.serial('Inspeção de Pneus — Configurações de Intervalo (Alexandre)', () => {
  ...
  test('B.3 Campo de pneus tem atributo min="0"', async ({ page }) => {
    await page.goto('/settings');
    await page.waitForLoadState('networkidle');
    ...
    // Verificar min=0 em algum input numérico da página de settings
    const inputWithMin0 = page.locator('input[type="number"][min="0"]');
    await expect(inputWithMin0).toBeVisible({ timeout: 5000 });
  });

  // ── C: Validação de mínimo ────────────────────────────────────────────────

  test('C.1 Salvar com valor 0 é permitido e persiste', async ({ page }) => {
    await page.goto('/settings');
    await page.waitForLoadState('networkidle');
    ...
    const inputWithMin0 = page.locator('input[type="number"][min="0"]').first();
    const inputVisible = await inputWithMin0.isVisible({ timeout: 5000 }).catch(() => false);
    if (!inputVisible) {
      test.skip(true, 'Input com min=0 não encontrado');
      return;
    }

    // Salvar com valor 0
    await inputWithMin0.fill('0');
    ...
    // Verificar que valor 0 persiste
    await expect(pneusLabel).toBeVisible({ timeout: 15000 });

    const inputAfterReload = page.locator('input[type="number"][min="0"]').first();
