...
const VEHICLES = [
  ...
  },
  {
    ...
    type: 'Passeio',
    energy: 'Elétrico',
    owner: 'Arrendamento VIP',
    warranty: true,
    warrantyEnd: '2030-10-10',
    hasInsurance: true,
    hasMaintenance: false
  },
  {
...
test.describe('Seeding Heavy (Full): Cadastro de Veículos via Analista', () => {
  ...
  test('Cadastrar Todos os Veículos (Analyst Bypass)', async ({ page }) => {
    for (const v of VEHICLES) {
      ...
      if (alreadyExists) {
        console.log(`Veículo ${v.plate} já existe. Pulando.`);
        continue;
      }
      ...
      await modal.locator('input[name="fipePrice"]').fill('150.000,00');
      await modal.locator('input[name="tracker"]').fill('Ituran');
      await modal.locator('input[name="antt"]').fill('987654');
      await modal.locator('input[name="autonomy"]').fill('400');
      ...
      if (v.energy === 'Combustão') {
        ...
        await modal.locator('input[name="tankCapacity"]').fill('100');
        await modal.locator('input[name="avgConsumption"]').fill('7,0');
      }

      await modal.locator('input[name="pbt"]').fill('5,0');
      await modal.locator('input[name="cmt"]').fill('60,0');
      await modal.locator('input[name="eixos"]').fill('3');
