...
- Uma OS de manutenção só pode gerar novas parcelas quando `budget_status = 'aprovado'` e `status` está em `('Concluído', 'Veículo retirado')`. Parcelas legadas já lançadas continuam sendo exibidas no ledger.
