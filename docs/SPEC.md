...
`action_plans` tem **6 policies** (migration `20260914100000_yard_auditor_action_plan_responsible.sql`): as 4 gerais (`action_plans_select/insert/update/delete`) e 2 aditivas para o `Yard Auditor` (`action_plans_select_yard_auditor_responsible`, `action_plans_update_yard_auditor_responsible`), que recortam por `responsible_id = auth.uid()`, tenant e papel. O gatilho `BEFORE UPDATE` `fn_enforce_yard_auditor_action_plan_update` (SECURITY INVOKER; age só quando quem altera é Yard Auditor) aceita apenas as colunas `status`, `claimed_by`, `claimed_at`, `conclusion_evidence_url`, `completion_notes` e `updated_at`, apenas as transições `pending → in_progress` (com `claimed_by = auth.uid()`) e `in_progress → awaiting_conclusion` (evidência restrita a `<client_id>/action-plans/<plan_id>/evidence.*`), e recusa o resto com `YARD_AUDITOR_*` (`42501`). Como o Auditor não lê `profiles`/`checklists` de terceiros, os nomes exibidos vêm da RPC `get_yard_auditor_action_plan_labels(p_action_plan_ids uuid[])` — SECURITY DEFINER, `search_path = public`, projeção só de nomes/títulos, filtro de responsável/tenant/papel reimposto e EXECUTE revogado de `anon` (mesmo padrão de `get_payment_installment_auditors`). Verificação estrutural: `supabase/diagnostics/check-yard-auditor-action-plan.sql`; comportamento: `e2e/pending/action-plan-auditor-flow.spec.ts` (projeto `auditor`).
...
### Trava de orçamento na mudança de status (migration `20260920000000_enforce_budget_gate_on_maintenance_status.sql`)

Três colunas aditivas e nullable em `maintenance_orders` — `budget_override_reason` (`TEXT`), `budget_override_by_id` (`UUID` → `profiles`, constraint literal `maintenance_orders_budget_override_by_id_fkey`) e `budget_override_at` (`TIMESTAMPTZ`) — registram a exceção de orçamento. `fn_enforce_maintenance_budget_gate` (`BEFORE INSERT OR UPDATE`, `SECURITY DEFINER`, `search_path=public`) aplica as Regras A e C descritas em "Regras de negócio críticas".

- **A exceção é cobrada uma única vez, na ENTRADA da faixa operacional** — nunca em cada transição. Isso é deliberado: `autoRetireVehicleFromWorkshop` (`src/lib/workshopScheduleUtils.ts`) move a OS de `Concluído` para `Veículo retirado` **sem humano no caminho**, disparada pelo checklist de Saída de Oficina do motorista, possivelmente offline. Cobrar motivo em toda transição quebraria esse fluxo. Um agente futuro **não deve** "completar" a regra estendendo-a às transições internas.
- **A autoria é carimbada pelo banco.** `budget_override_by_id := auth.uid()` e `budget_override_at := now()` sobrescrevem o que o cliente enviar — não são forjáveis.
- **Os três campos são imutáveis fora da transição qualificada.** Em qualquer outro INSERT ou UPDATE, o gatilho os restaura a partir de `OLD` (ou anula, no INSERT). A sobrescrita é **silenciosa**, não `RAISE`: a aplicação nunca envia essas colunas fora da porta, então um erro ali só produziria falha inexplicável para o usuário.
- **`Orçamento aprovado` é o único status sem porta de exceção.** Os demais descrevem um fato do mundo — o serviço está acontecendo —, que pode ser verdadeiro sem orçamento. `Orçamento aprovado` é uma afirmação sobre um ato que ou ocorreu no Financeiro ou não ocorreu; nenhuma justificativa a torna verdadeira, e o campo alimenta `isOrderPayable` e o teto de parcelas da OS.
- **Escape hatch:** com `auth.uid() IS NULL` (SQL Editor / `service_role`) o gatilho devolve `NEW` sem validar — mesmo padrão de `fn_enforce_maintenance_cancellation_reason`. Consequência a observar: qualquer job futuro que rode com `service_role` e escreva `status` passa por fora das Regras A e C.
- **Sem backfill.** As OS que já estavam na faixa operacional sem orçamento quando a migration foi aplicada ficam com os três campos nulos e exibem `Não informado` / `Não identificado`. A auditoria vale a partir da aplicação.
- **Contrato de aplicação:** `src/lib/maintenanceBudgetOverride.ts` é a fonte única de constantes, mensagens e formatação, e delega a normalização a `normalizeReasonText` (`src/lib/reasonText.ts`). A query de `/manutencao` traz o autor pelo embed `budget_override_by:profiles!maintenance_orders_budget_override_by_id_fkey (name)`, mapeado para `budgetOverrideByName` — sujeito à mesma limitação do papel `Workshop` descrita acima.
- **Rollback** em `supabase/migrations/rollback/20260920000000_rollback_budget_gate_on_maintenance_status.sql` (derruba gatilho e função; **não** remove as colunas, que podem conter motivos reais). **Diagnóstico** somente-leitura em `supabase/diagnostics/check-maintenance-budget-gate.sql`.
...
- `fn_validate_extra_payment_request_transition` (BEFORE UPDATE em `extra_payment_requests`): aprova/reprova (`Coordinator+`/`Admin Master`; reprovação exige motivo; aprovação exige ao menos uma parcela, todas pendentes e soma igual a `amount`) e só valida/preenche auditoria — não toca nas parcelas nessa mesma transição. `pendente_aprovacao|aprovado → cancelado` exige motivo; de `aprovado`, só por quem aprovou o pedido (`approved_by`) ou Admin Master, com trava `FOR UPDATE` das parcelas e recusa se alguma estiver `pago`. Pedido `cancelado` é imutável.
- `fn_sync_extra_payment_request_installments` (AFTER UPDATE OF status em `extra_payment_requests`): propaga `pendente_aprovacao → aprovado|reprovado` para todas as parcelas pendentes do pedido e, desde 2026-09-11, `→ cancelado` para as parcelas pendentes/aprovadas, copiando o motivo.
...
- `fn_enforce_payment_installment_budget_cap` (teto de orçamento) segue só se aplicando quando `source_type = 'maintenance_order'`. Parcelas `cancelado` liberam saldo, como `reprovado` (`sumCommittedValue`/`countCommittedInstallments` no frontend).
- `maintenance_orders.status` e `maintenance_orders.budget_status` devem permanecer coerentes, em três regras (migration `20260920000000`, gatilho `fn_enforce_maintenance_budget_gate`; no frontend, `canAdvanceMaintenanceStatus` e `requiresBudgetOverrideReason` em `src/lib/maintenanceStatusCoherence.ts`):
  - **Regra A — bloqueio duro.** `status = 'Orçamento aprovado'` só é gravável quando `budget_status = 'aprovado'`. Vale em INSERT e em UPDATE, e **não há motivo que libere**: o status é consequência exclusiva da aprovação em Financeiro → Aprovação de Orçamentos.
  - **Regra B — inalterada.** Uma OS com orçamento `pendente` ou `reaberto` não entra nem anda em `Serviço em execução`, `Concluído` ou `Veículo retirado` (`fn_enforce_maintenance_status_budget_coherence`, migration `20260827000000`). O bloqueio é duro e tem **precedência** sobre a exceção da Regra C.
  - **Regra C — exceção auditada.** **Entrar** na faixa operacional com orçamento `sem_orcamento` ou `reprovado` exige motivo escrito não-vazio de até 500 caracteres; o banco carimba autor e data. "Entrar na faixa" significa `status` anterior **fora** de `('Serviço em execução','Concluído','Veículo retirado')` — inclusive o INSERT, que não tem status anterior — e `status` novo **dentro** dela. Transições **dentro** da faixa não exigem nada.
  - Uma OS com orçamento `aprovado` não retorna para `Aguardando aprovação`.
- Uma OS de manutenção só pode gerar novas parcelas quando `budget_status = 'aprovado'` e `status` está em `('Orçamento aprovado', 'Serviço em execução', 'Concluído', 'Veículo retirado')` (ampliado em 2026-09-10, migration `20260910000000`; antes só `Concluído`/`Veículo retirado`). `Cancelado` e os status anteriores à aprovação continuam recusados. A regra vive em `PAYABLE_MAINTENANCE_STATUSES` (frontend) e em `fn_enforce_payment_installment_source_payable` (banco). Como o lançamento pode anteceder a conclusão, o status operacional da OS é exibido no card de aprovação e na coluna Status de `PaymentsTab` (`src/lib/maintenanceOrderPaymentSignal.ts`), e o cancelamento de OS com parcelas não reprovadas exige confirmação informada. Parcelas legadas já lançadas continuam sendo exibidas no ledger.
- A planilha XLSX de `PaymentsTab` nunca inclui parcela `cancelado`, mesmo selecionada (`resolveExportSelection`).
- O card "Total do mês" de `ExtraPaymentsTab` exclui pedidos `reprovado` e `cancelado` (`sumExtraPaymentMonthTotal`).
...
### RLS
`extra_payment_requests`: SELECT para Fleet Assistant+ do tenant e Admin Master (todos os status); Financeiro do tenant só vê `aprovado`/`pago` e, desde 2026-09-11, `cancelado` com `approved_at IS NOT NULL` (o mesmo vale para as parcelas desses pedidos em `payment_installments_select`). INSERT restrito a Fleet Assistant+ exceto Workshop/Financeiro, sempre `pendente_aprovacao` e `created_by_id = auth.uid()`. UPDATE para Coordinator+/Admin Master (aprovar/reprovar/cancelar) e para o próprio criador cancelar (`pendente_aprovacao → cancelado`). Sem policy de DELETE.

`payment_installments`: INSERT para Fleet Assistant+ do tenant, Admin Master e Workshop na OS da própria oficina, sempre com `created_by_id = auth.uid()` e, **desde 2026-09-12 (migration `20260912000000`), sempre `status = 'pendente_aprovacao'` com os sete campos de auditoria vazios** (`payment_approved_by/at`, `paid_by/at`, `cancelled_by/at`, `cancellation_reason`). Antes disso a policy não olhava o `status`, e como `fn_validate_payment_installment_transition` é só `BEFORE UPDATE`, qualquer papel com porta de INSERT podia criar parcela já `aprovado` ou `pago` por chamada direta à API, pulando a aprovação do Coordenador — brecha aberta desde 2026-07-08 e nunca explorada (0 de 53 parcelas em PROD). A trava é **por policy, não por gatilho**, seguindo o padrão já existente em `extra_payment_requests_insert`; `service_role` não passa por RLS, então reparo manual pelo SQL Editor continua liberado. Verificação estrutural: `supabase/diagnostics/check-payment-installment-policies.sql`; verificação de comportamento: `e2e/pending/payment-installment-insert-status.spec.ts`. **A porta do papel `Workshop` é desenho, não resíduo (decisão do usuário, 2026-09-12).** A interface acompanha a policy: a oficina vê o item Financeiro no menu, acessa `/financeiro`, vê a aba Pagamentos e tem o botão "Cadastrar Pagamento" (`ROLES_CAN_CREATE_PAYMENTS` inclui `Workshop`). Ela também tem porta de `UPDATE` e de `DELETE` sobre parcela `pendente_aprovacao` das OS da própria oficina, inclusive parcela criada pela equipe do cliente; a cláusula `status = 'pendente_aprovacao'` no `DELETE` é o que impede exclusão de parcela aprovada ou paga, e o gatilho de transição é o que impede edição depois de aprovada. **`extra_payment_requests_insert` exclui `Workshop` de propósito** — pagamento extra não tem OS de oficina associada — e a assimetria entre as duas tabelas é intencional: **um agente futuro não deve "uniformizar" as duas.** O contrato de papéis está travado por testes unitários em `src/lib/rolePermissions.test.ts`.

### Outras RPCs
- `next_extra_payment_request_number(p_client_id)`: gera o próximo número sequencial por mês/cliente.
- `get_extra_payment_auditors(p_extra_payment_request_id)`: nomes de auditoria (criado/aprovado/reprovado/pago/cancelado por — `cancelled_by_name`), `SECURITY DEFINER` reimpondo visibilidade por tenant/status.
- `get_payment_installment_auditors(p_installment_id)`: nomes de auditoria de uma parcela de manutenção, incluindo `cancelled_by_name`, `SECURITY DEFINER`.
