...
- `maintenance_orders.status` e `maintenance_orders.budget_status` devem permanecer coerentes: uma OS com orçamento `pendente` ou `reaberto` não pode avançar para `Serviço em execução`, `Concluído` ou `Veículo retirado`; orçamento `sem_orcamento` ou `reprovado` não bloqueia a operação, e uma OS com orçamento `aprovado` não retorna para `Aguardando aprovação`.
