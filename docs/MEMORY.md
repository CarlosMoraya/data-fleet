...
- **Manutenção**: workflow de OS, cancelamento, aprovação de orçamento e OCR operacional. **Paridade de piso de KM com o checklist (2026-06-22)**: o campo "Km Atual do Veículo" do modo padrão do `MaintenanceForm` passou a aplicar a MESMA regra de piso do checklist, reusando a função pura `validateChecklistOdometerKm` via wrapper `validateMaintenanceCurrentKm` (`src/lib/maintenanceKmValidation.ts`). O `referenceKm` vem da RPC `get_vehicle_max_effective_km` com fallback em `vehicles.initial_km`, exatamente como o `ChecklistFill`. O hint "Último Km registrado: X km" aparece sob o campo no modo padrão. O campo continua **opcional** (vazio = válido); o modo Workshop permanece **intacto** (fora do escopo). A OS não retroalimenta a fonte de KM efetivo — a validação apenas lê o piso.
