...
- **Migrations da sub-aba Aderência NÃO aplicadas em nenhum banco (2026-08-19)**: `supabase/migrations/20260819000000_add_auditoria_day_interval.sql` (rollback em `supabase/migrations/rollback/`) e a alteração do `IN` da RPC em `20260619000005_recreate_dashboard_last_checklist_rpc.sql` + `apply-dashboard-rpcs-production.sql`. Aplicar em DEV primeiro; a validação manual guiada de 12 passos está bloqueada até lá — em especial o passo 11, que confere que os números do Dashboard e de Veículos não mudaram.
