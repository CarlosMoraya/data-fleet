...
0. `20260803000000_add_budget_discounts.sql` (desconto no orçamento de manutenção, 2026-08-03) — migration criada pelo agente, **ainda não executada em nenhum ambiente**. Aplicar no Supabase DEV primeiro, validar por SQL manual (colunas `maintenance_budget_items.discount` e `maintenance_orders.budget_discount` existem; `CHECK` e 2 `TRIGGER` ativos; `NOTIFY pgrst` executado). Só então promover a PROD. Após aplicar em DEV, executar a validação manual guiada de 11 passos descrita em `IMPLEMENTATION.md` § "Validação manual guiada".
