...
Implementado exatamente o escopo fechado de `IMPLEMENTATION.md`: novo campo opcional por default no cadastro de Veículos, configuração de obrigatoriedade por tenant, exibição no detalhe, aviso não bloqueante de incoerência e exportação das duas datas no XLSX. Nenhuma dependência foi instalada nem comando SQL foi executado pelo agente. Após a implementação, o usuário informou que aplicou a migration em PROD; o estado de DEV não foi alterado nesta sessão.

**Banco e domínio** — Criados `supabase/migrations/20260811000000_add_vehicle_operation_start_date.sql` e `supabase/migrations/rollback/20260811000000_add_vehicle_operation_start_date_rollback.sql`. A migration adiciona `vehicles.operation_start_date` nullable e `vehicle_field_settings.operation_start_date_optional boolean NOT NULL DEFAULT true`, com os comentários especificados. O usuário informou que aplicou a migration em PROD após a implementação; nenhuma aplicação foi executada pelo agente, não houve backfill ou alteração de RLS e o estado de DEV não foi alterado nesta sessão. O campo foi adicionado às interfaces `Vehicle` e `VehicleFieldSettings`, aos mappers banco ↔ domínio e ao allowlist de rascunhos `bf:v1:ui`.
...
**Pendências** — A validação manual guiada de 10 passos só pode ser executada depois que o usuário aplicar a migration em DEV. A árvore já estava suja antes da sessão; alterações pré-existentes em `src/pages/Vehicles.tsx`, `VehicleForm.tsx` e `VehicleDetailModal.tsx` foram preservadas, e os módulos de garantia e o teste de export de Manutenção permaneceram fora do escopo desta sessão.
